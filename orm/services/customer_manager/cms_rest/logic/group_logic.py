...
class GroupLogic(object):
    ...
    def unassign_roles(self,
        ...
        try:
            ...
            cms_role_record = datamanager.get_record('cms_role')
            role_id = cms_role_record.get_cms_role_id_from_name(role_name)

            for group_region in groups_regions:
                region_id = group_region.region_id

                if assignment_type == "domain":
                    role_record = datamanager.get_record('groups_domain_role')
                    role_record.remove_domain_role_from_group(
                        group_id, region_id, assignment_value, role_id)

                    groups_role.remove_role_from_group(group_id, role_id)

                elif assignment_type == "customer":
                    customer_id = datamanager.get_customer_id_by_uuid(
                        assignment_value)

                    role_record = datamanager.get_record('groups_customer_role')
