...
class RoleController(rest.RestController):
    ...
    @wsexpose(RoleResultWrapper, str, body=[Role],
              rest_content_types='json', status_code=200)
    def post(self, group_id, roles):
        LOG.info("RoleController - Assign Roles to group id {0} "
                 "roles: {1}".format(group_id, str(roles)))
        ...
        try:
            ...
            event_details = 'Group {} - Assigned roles: {}'.format(
                group_id, [r.role for r in roles])
    ...
    @wsexpose(None, str, body=[Role],
              rest_content_types='json', status_code=204)
    def delete(self, group_id, roles):
