...
def calculate_name(flavor):
    ...
    # if series in 'p1':
    if series in 'p1':
        if {'n0', 'n1'}.issubset(flavor.flavor.options.keys()):
            raise ConflictError(409, "Flavor options n0 and n1 cannot be used together")

        if {'i2'}.issubset(flavor.flavor.options.keys()) and not \
                {'n0'}.issubset(flavor.flavor.options.keys()):
    ...
    if len(flavor.flavor.options) > 0:
        for key in sorted(flavor.flavor.options.iterkeys()):
            ...
            if (series == 'p1' and key in valid_p1_opts):
