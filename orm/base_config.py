...
config = ConfigParser()
config.read('/etc/ranger/ranger.conf')

valid_flavor_series = config.get('fms', 'flavor_series')
valid_flavor_series = [x.strip() for x in
                       config.get('fms', 'flavor_series').split(',')]
